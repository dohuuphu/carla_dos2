...
class yolo():
    def __init__(self,
        weights='/mnt/HDD1/phudh/course/calar/IDS_s24/phu_dev/agents/TFPP/yolov9/weights/yolov9-c-converted.pt',  # model path or triton URL
        data='/mnt/HDD1/phudh/course/calar/IDS_s24/phu_dev/agents/TFPP/yolov9/data/coco.yaml',  # dataset.yaml path
    ...
    def forward(self, im0:np.ndarray, timestamp,
                augment=False, 
                conf_thres=0.25,  # confidence threshold
                iou_thres=0.45,  # NMS IOU threshold
                max_det=1000,
        ...
        im0 = cv2.cvtColor(im0 , cv2.COLOR_RGBA2RGB) #BGR
        ...
        for i, det in enumerate(pred):  # per image
            ...
            save_path = f'./{timestamp}_{i}_pred.jpg'  # im.jpg
            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
            imc = im0.copy() if self.save_crop else im0  # for save_crop
            annotator = Annotator(im0, line_width=self.line_thickness, example='yov9_predict')
            if len(det):
