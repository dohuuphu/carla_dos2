...
class LidarCenterNet(nn.Module):
  ...
  def visualize_model(  # pylint: disable=locally-disabled, unused-argument
    ...
    if pred_semantic is not None:
      pre_semantic_image = pred_semantic[0].detach().cpu().numpy()
      c,h,w = pre_semantic_image.shape
      class_map = np.argmax(pre_semantic_image, axis=-3)
      rgb_image_semantic = np.zeros((h, w, 3), dtype=np.uint8)
      for i, color in enumerate(colors):
        rgb = np.array(plt.get_cmap(cmap)(i)[:3]) * 255
        rgb_image_semantic[class_map == i] = rgb
      all_images = np.concatenate((rgb_image_semantic, all_images), axis=0)
    if pred_depth is not None:
      depth_image = pred_depth.detach().cpu().numpy()[0]
      depth_rgb = (np.stack((depth_image, depth_image, depth_image), axis=-1)*255).astype(np.uint8)    
      all_images = np.concatenate((depth_rgb, all_images), axis=0)
